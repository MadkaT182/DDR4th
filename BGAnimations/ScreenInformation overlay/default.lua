...
for i=1,#infoPages do
	infoItems[#infoItems+1] = Def.ActorFrame{
		Def.ActorFrame{
			...
			Def.Quad{
				InitCommand=cmd(addx,selectorOffset;y,-8;zoomto,581,24;diffuse,color("0,0,0,1"));
			};
			LoadActor("_selector")..{
				Name="Selector";
				InitCommand=cmd(addx,selectorOffset);
			};
			LoadFont("_green solid")..{
				Name="IndexText";
				InitCommand=cmd(x,250;addx,selectorOffset;y,14;queuecommand,"Set";diffuseblink;effectcolor1,color("1,1,1,1");effectcolor2,color("#EFB500");effectperiod,0.25);
